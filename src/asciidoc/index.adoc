...
These are example outputs for the `GET /auth/refresh` endpoint for bad request.
...
.request
include::auth/logout/http-request.adoc[]

.response
include::auth/logout/http-response.adoc[]

It logs out the authenticated user by invalidating their refresh token.
...
.request
include::auth/logout-missing-token/http-request.adoc[]

.response
include::auth/logout-missing-token/http-response.adoc[]
...
.request
include::auth/logout-expired-token/http-request.adoc[]

.response
include::auth/logout-expired-token/http-response.adoc[]
...
==== 1.5.2 Error Response - 401 - Unauthorised
...
This is an example output for the `DELETE /users/{userId}` endpoint - soft delete.
...
This is an example output for the `DELETE /users/{userId}/permanent` endpoint.
...
This is an example output for the `PUT /users/{userId}/restore` endpoint.
