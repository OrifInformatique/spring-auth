...
API documentation is generated by **Spring REST Docs** from the MockMvc integration tests.

1. Tests in `AuthControllerIntegrationTest` and `UserControllerIntegrationTest` call `document(...)`.
2. Spring REST Docs writes HTTP snippets under `target/generated-snippets/`.
3. Happy-path tests also declare `requestFields` / `responseFields`. If a JSON field is added, removed or renamed, the test fails instead of producing outdated docs.
4. `src/asciidoc/index.adoc` includes those snippets via `{snippets}`.
5. The Asciidoctor Maven plugin builds `docs/index.html` during `mvn package` (phase `prepare-package`).
...
- AsciiDoc source: [src/asciidoc/index.adoc](src/asciidoc/index.adoc)
